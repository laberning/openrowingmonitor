...
function createFlywheel (rowerSettings) {
  ...
  function pushValue (dataPoint) {
    ...
    _angularAccelerationAtBeginFlank = _angularAcceleration.median()
  ...
  function markRecoveryPhaseCompleted () {
    ...
    if (rowerSettings.autoAdjustDragFactor && recoveryDeltaTime.length() > minimumDragFactorSamples && recoveryDeltaTime.goodnessOfFit() >= rowerSettings.minimumDragQuality && recoveryDeltaTime.slope() > 0) {
      ...
      if (rowerSettings.autoAdjustRecoverySlope) {
        ...
        minumumRecoverySlope.push(-1 * (1 - rowerSettings.autoAdjustRecoverySlopeMargin) * recoveryDeltaTime.slope())
