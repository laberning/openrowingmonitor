...
function createTSQuadraticSeries (maxSeriesLength = 0) {
  ...
  function calculateC (pointOne, pointThree) {
    if ((pointOne + 1) < pointThree && X.get(pointOne) !== X.get(pointThree)) {
      ...
      while (pointOne < pointTwo && pointTwo < pointThree && X.get(pointOne) !== X.get(pointTwo) && X.get(pointTwo) !== X.get(pointThree)) {
        // For the underlying math, see https://www.quora.com/How-do-I-find-a-quadratic-equation-from-points/answer/Robert-Paxson
        results.push((Math.pow(X.get(pointOne), 2) * (X.get(pointTwo) * Y.get(pointThree) - X.get(pointThree) * Y.get(pointTwo)) + Math.pow(X.get(pointTwo), 2) * (X.get(pointThree) * Y.get(pointOne) - X.get(pointOne) * Y.get(pointThree)) + Math.pow(X.get(pointThree), 2) * (X.get(pointOne) * Y.get(pointTwo) - X.get(pointTwo) * Y.get(pointOne))) / ((X.get(pointOne) - X.get(pointTwo)) * (X.get(pointOne) - X.get(pointThree)) * (X.get(pointTwo) - X.get(pointThree))))
        pointTwo += 1
      }
      return results.median()
    } else {
