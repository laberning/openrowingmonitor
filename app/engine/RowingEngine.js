...
/*
  ...
  Based on their great work, we created a simplified and more robust Physics Engine
*/
...
function createRowingEngine(rowerSettings) {
  ...
  const flankDetector = createMovingFlankDetector(
    rowerSettings.numOfImpulsesPerRevolution,
    rowerSettings.maximumTimeBetweenMagnets,
    0
  );
  ...
  function handleRotationImpulse(currentDt) {
    ...
    if (
      currentDt < rowerSettings.minimumTimeBetweenMagnets ||
      currentDt > rowerSettings.maximumTimeBetweenMagnets ||
      currentDt <
        rowerSettings.maximumDownwardChange * PreviousDt.weightedAverage() ||
      currentDt >
        rowerSettings.maximumUpwardChange * PreviousDt.weightedAverage()
    ) {
      // impulses are outside plausible ranges, so we assume it is close to the previous one
      currentDt = prevDt;
      log.debug(
        `Noisefilter corrected currentDt, ${currentDt} was incredible, changed to ${prevDt}`
      );
    }
    prevDt = currentDt;
    ...
    if (liquidFlywheel) {
      ...
      if (isInDrivePhase && !wasInDrivePhase) {
      ...
      if (wasInDrivePhase) {
        ...
        if (
          strokeElapsed > rowerSettings.minimumDriveTime &&
          flankDetector.isDecelerating()
        ) {
        ...
        if (
          recoveryElapsed > rowerSettings.minimumRecoveryTime &&
          flankDetector.isAccelerating()
        ) {
    ...
    log.debug(
      `𝑑t: ${currentDt} ω: ${omegaVector[0].toFixed(
        2
      )} ωdot: ${omegaDotVector[0].toFixed(2)} ωdotdot: ${omegaDotDot.toFixed(
        2
      )} aPos: ${accelerationIsPositive} aChange: ${accelerationIsChanging}`
    );
  }
  ...
  function startDrivePhase(currentDt) {
    ...
    if (strokeElapsed - driveElapsed !== 0) {
      kDampEstimatorAverager.pushValue(
        kDampEstimator / (strokeElapsed - driveElapsed)
      );
    }
    log.debug(
      `estimated kDamp: ${
        jMoment * (-1 * kDampEstimatorAverager.weightedAverage())
      }`
    );
    log.info(
      `estimated omegaDotDivOmegaSquare: ${
        -1 * kDampEstimatorAverager.weightedAverage()
      }`
    );
  ...
  function updateDrivePhase(currentDt) {
    ...
    kPower =
      kPower +
      kDamp * (omegaVector[0] * omegaVector[0] * omegaVector[0]) * currentDt;
  ...
  function updateRecoveryPhase(currentDt) {
    kDampEstimator =
      kDampEstimator +
      (omegaDotVector[0] / (omegaVector[0] * omegaVector[0])) * currentDt;
  }
