...
```mermaid
flowchart LR
A(GpioTimerService.js) -->|time between ticks| B(server.js) & B(server.js) -->|Heart rate data| D(RowingStatistics.js) & D(RowingStatistics.js) -->|Rowing metrics| B(server.js)
